...
# Development script for ankurah.org example
...
echo -e "${GREEN}Starting ankurah.org example development environment...${NC}\n"
...
cleanup() {
    ...
    pkill -f "ankurah-org-example-server" 2>/dev/null || true
...
cargo run --release -p ankurah-org-example-server 2>&1 | sed 's/^/[SERVER] /' &
SERVER_PID=$!
PIDS+=($SERVER_PID)
